...
async def start_exam(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(available) < 5:
        await update.message.reply_text("❗ Недостаточно вопросов для экзамена." if lang == "ru"
                                        else "❗ Not enough questions for the exam.")
        return

    exam_questions = random.sample(available, 5)
    await update.message.reply_text("🎓 Экзамен начался! Ты услышишь 5 вопросов. На каждый — 29 секунд." if lang == "ru"
                                    else "🎓 Exam started! You will hear 5 questions. You have 29 seconds for each.")

    for idx, question in enumerate(exam_questions, 1):
        await update.message.reply_text(f"📝 Вопрос {idx}: {question['question_en']}" if lang == "ru"
    ...
    await update.message.reply_text("✅ Экзамен завершён! Молодец!" if lang == "ru"
